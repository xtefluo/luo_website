...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(number):
            if i % 50 == 0:
                self.stdout.write(self.style.SUCCESS(f'正在添加第{i + 1}个虚构数据'))
            ...
            if status == 'SU':
                ...
                fee_price = Decimal(0)
            refund_price = course.price if status == 'PE' else Decimal(0)
            ...
            order.save()
        self.stdout.write(self.style.SUCCESS(f'成功添加{number}虚构订单数据'))